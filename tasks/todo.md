...
## Objective
- Create a new GitHub repo for **Florentine Groteschi**.
...
- Include the current Florentine Groteschi artwork/metadata outputs in the repo.
...
- New GitHub repository `francoisjvr/florentine-groteschi`.

## Risk Class
- medium: new contract repo plus admin burn feature, but no deployment or on-chain transaction in scope.

## Assumptions / Unknowns
- Collection name: Florentine Groteschi.
- Token symbol: FGROT.
- Max supply: 1,401, matching the generated Florentine Groteschi art plan.
- Current repo will include the latest 24-piece final-polish artwork preview batch plus metadata/render report; full 1,401 production artwork can be added later when generated.
...
- Exact rollback steps:
  - Local: revert or delete `C:/Users/Francois/HermesWorkspace/florentine-groteschi`.
  - Remote: use `gh repo delete francoisjvr/florentine-groteschi` only if explicitly requested; otherwise push a corrective commit.
- Validation after rollback: `gh repo view francoisjvr/florentine-groteschi` or local `git status`.

## Implementation Checklist
- [x] Inspect source contracts and identify burn/admin pieces.
- [x] Scaffold Florentine Groteschi repo from Normie Pets project structure.
...
## Review
- Summary: OpenSea SeaDrop-compatible Florentine Groteschi repo created, agent burn integrated, current artwork preview included, audit written, and GitHub push verified.
- Evidence: `npm test` = 14 passing; `npm run compile` = success; production npm audit = 0 vulnerabilities; deployed bytecode = 12,066 bytes; secret scan = 0 findings; remote = `https://github.com/francoisjvr/florentine-groteschi`.
